...
//    donne un entier non nul, la fonction holdall_put effectue une insertion en queue. Dans
...
void holdall_dispose_all(holdall **haptr){
  if (*haptr == NULL) {
    return;
  }
  choldall *p = (*haptr)->head;
...
int holdall_add(holdall *ha, void *ref) {
  choldall *p = malloc(sizeof *p);
  if (p == NULL) {
    return -1;
  }
